...
function buildClaudePrompt(companyName, articles, businessContext) {
...
A software vendor has a B2B customer called "${companyName}".
${businessContext ? `Context about this customer: ${businessContext}` : ''}

Analyze these recent news articles about ${companyName}.
Determine if any article indicates this customer is at risk 
of cancelling their B2B software subscription.

ONLY flag as churn-relevant if the news directly suggests:
- Layoffs or workforce reductions affecting their software budget
- Acquisition where new owners audit and cut vendor contracts  
- CEO or key executive departure who championed this software
- Budget cuts or cost reduction programs
- Bankruptcy or severe financial distress
- Major restructuring changing their technology needs

Do NOT flag as churn-relevant:
- General company news unrelated to their ability to pay
- Product launches or new features they are releasing
- News about their own customers
- Stock price movements without financial distress
- Awards, rankings, or positive growth news
- Industry trends that don't affect this specific company

Articles:
${articleLines}

Respond ONLY with valid JSON no markdown no explanation:
{"churn_relevant":true,"risk_score":75,"risk_reason":"specific one sentence reason","recommended_action":"specific action to take this week","urgency":"immediate","signal_category":"layoffs"}

urgency must be one of: immediate, this_week, monitor, none
signal_category must be one of: layoffs, acquisition, leadership_change, financial_distress, restructuring, none`;
}
    ...
    try {
      ...
      if (articles.length === 0) {
        ...
        const businessContext =
          contacts[0]?.business_description ||
          (contacts[0]?.contact_email
            ? `Primary contact email domain: ${(contacts[0].contact_email || "").split("@")[1]}`
            : "");

        const prompt = buildClaudePrompt(displayName, articles, businessContext);
